...
# Define number of trees to try — from 1 to 100 in steps
ntree_seq <- c(1, 5, 10, 20, 30, 40, 50, 60, 75, 100)

# Initialize results storage
results <- tibble(ntree = ntree_seq, accuracy = NA_real_)

# Loop through ntree values
for (i in seq_along(ntree_seq)) {
  # Train model
  model <- ranger(quality ~ ., data = train_dat, num.trees = ntree_seq[i], probability = FALSE)
